...
class Mails:
    ...
    def check_users_and_groups(names_list):
        ...
        for name_str in names_list:
            ...
            if name_str and isinstance(name_str, str) and '@' not in name_str:
                ...
                # Check user adprod values too!
                # User name in django usually differ from user adprod account name.
                # ADPROD value is added by Octopus Admin into the related column manually when user register first.
                elif User.objects.filter(user_profile__adprod_username__exact=name_str).exists():
                    user_email = User.objects.filter(user_profile__adprod_username__exact=name_str).first().email
                    validated_emails_list.append(user_email)
    ...
    def short(self, **mail_args):
        ...
        attach_content_name = mail_args.get('attach_content_name', 'trianglesis.html')
